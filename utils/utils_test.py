...
def split_tree_into_feature_groups(tree, max_tree_depth: int) -> (np.ndarray, np.ndarray, np.ndarray):
    ...
    actions_diat={"L":1,"F":2,"R":3,"B":4}
    max_input=max_tree_depth
    # for direction in TreeObsForRailEnv.tree_explored_actions_char:
    ...
    i=1
    queue.append((0,tree))
    while len(queue)>0 and i<max_input:
        action,tmp_node=queue.pop(0)
        for direction in TreeObsForRailEnv.tree_explored_actions_char:
            if action==0:
                new_action=actions_diat[direction]
            else:
                new_action=action
            if hasattr(tmp_node,"childs") and isinstance(tmp_node.childs,dict) and direction in tmp_node.childs.keys():
                node=tmp_node.childs[direction]
                if node != -np.inf:
                    queue.append((new_action,node))
                    sub_data, sub_distance, sub_agent_data = _split_node_into_feature_groups(node)
                    data = np.concatenate((data, np.append(sub_data,[new_action])))
                    distance = np.concatenate((distance, sub_distance))
                    agent_data = np.concatenate((agent_data, sub_agent_data))
                    i=i+1
                    if i >=max_input:
...
def normalize_observation(observation, tree_depth: int, observation_radius=0):
    ...
    if len(normalized_obs)< 12*tree_depth-1:
        normalized_obs=np.concatenate((normalized_obs,[-1]*(12*tree_depth-1-len(normalized_obs))))
